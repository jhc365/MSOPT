...
def run_PLASynth(samples, outfile, outfile_excel="",count=0,timeout = 0):
    ...
    for i,tup in enumerate(samples):
        ...
        for m_num, module in enumerate(modules):
            ...
            #     continue
            simplified, semantic_eq, iter_count = module.simplify_restart(expr)
            # simplified, semantic_eq, iter_count = module.simplify(expr)
        ...
        tmpdict_first['itercount'] = iter_count+1
...
def run(synthesis_module_type, sample_type,num):
    ...
    filename = "./restart_result/xyntia/220907_xyntia_timeout60(%d).json" % (num)
    # filename = "./result_0124_finaltest/%s_%s_try2.json" % (synthesis_module_type, sample_type)
    run_PLASynth(samples=samples,outfile=filename,timeout=num)
