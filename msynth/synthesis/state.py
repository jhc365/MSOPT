...
class SynthesisState:
    ...
    def get_hamming2_score(self, oracle: SynthesisOracle, variables: List[Expr]) -> float:
        """
        ...
        #         log2(1 + abs(int(oracle_out) - int(oracle_output)))
        ...
        diffvec = [
            # calculate score for input array
            int(self._evaluate(inputs, variables)) ^ int(oracle_output)
            # walk over all input arrays in the oracle
            for inputs, oracle_output in oracle.synthesis_map.items()
        ]
        orsum = reduce(lambda x, y: x | y, diffvec)
        return hamming(orsum)


    def get_hamming_score(self, oracle: SynthesisOracle, variables: List[Expr]) -> float:
        """
        Calculates the state's score used as feedback for the synthesizer.
        ...
        return sum([
            # calculate score for input array
            hamming(int(self._evaluate(inputs, variables)), int(oracle_output))
            # walk over all input arrays in the oracle
            for inputs, oracle_output in oracle.synthesis_map.items()
        ])
