...
class Simplifier:
    ...
    represented as an abstract synsimplifytax tree (AST) from the root downwards
    ...
    def _is_suitable_simplification_candidate(self, expr: Expr, simplified: Expr) -> bool:
        ...
        # sem_eq_expr_simp = self.check_semantical_equivalence(expr, simplified)
        if self.enforce_equivalence and self.check_semantical_equivalence(expr, simplified) != z3.unsat: # Todo : 중복비교
            logger.debug(
    ...
    def simplify(self, expr: Expr) -> Expr:
        ...
        while True:
            ...
            for subtree in get_subexpressions(ast):
                # skip subtree if possibleb'Sorts (_ BitVec 1) and (_ BitVec 32) are incompatible'
