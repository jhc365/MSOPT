...
def IsUnsatTa(binstr, trace_index, trace_list, out_f):
    ...
    if isinstance(sym_pc, ExprCond):
        try:
            ...
            out_f.write("trace%d_%d:%s\n" % (trace_index,cond_index,repr(sym_pc.cond)))
            trace_index += 1
            # out_f.write(repr(sym_pc.cond))
            # out_f.write("\n")
...
def main():
    # path_dir = '/home/plas/work/2019_obfus/dimva'
    # out_path = '/home/plas/work/2019_obfus/output/dimva_pa.csv'
    # out_path2 = '/home/ubuntu/2019_obfus/trace/K_time3.csv'
    path_dir = './trace_source'
    out_path = './out_cond/dimva_pa.csv'
    ...
    for target in file_list:
        ...
        cond_name = target #currnet target name for outfile name
        ...
        ta_op_cnt = 0
        pa_op_cnt = 0
        out_f = open('./out_cond/' + target + "cond.txt", 'w')  # cond for MSOPT Outfile open
        for trace_index in cjmp_index_list:
            ...
            #     pa_op_cnt += is_op
            #     pa_dict[trace_list[trace_index].get_pa()] = is_op
            cal_k = FigureK(trace_list, trace_index, 15)
            bin_str = BinToStr(trace_list, cal_k, trace_index)
            is_op = IsUnsatTa(bin_str, trace_index, trace_list, out_f)
            pa_dict[trace_list[trace_index].get_pa()] = is_op
        merged_pa_dict = Counter(merged_pa_dict) + Counter(pa_dict)
        # time_t = timeit.default_timer() - start
        wr.writerow([pa_op_cnt, ta_op_cnt, pa_dict, len(cjmp_ta_list)])
        pa_dict.clear()
        out_f.close()  # cond for MSOPT outfile close
