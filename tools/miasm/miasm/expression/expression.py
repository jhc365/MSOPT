...
#
# 20210709
# from class ExprVisitorBase(object):
#
class ExprSynthVisitor(object):
    """
    Run simplifier
     from subexpression, recursively!
    """
    def __init__(self, simplifier, synthesizer):
        self.simplifier = simplifier
        self.synthesizer = synthesizer
        self.conflixt_cnt = 0
        self.mySimplifier = self.Simplify()
        self.representation = dict()
        self.rep_count = 0

    def simpler(self, expr):
        """
        Main logic: currently it does nothing
        """
        # print ("# ESC: run your synthize based simplification result with expr, here--TODO!")
        # if (expr.depth() <  5):
        #      return self.simplifier.simplify(expr) # simplifier should have simplify(expr) function
        # return self.visit(expr)

        # if (expr.depth() <  5):
        if len(self.get_unification_candidates(expr)) < 4:
            # result =  self.simplifier.simplify(expr) # simplifier should have simplify(expr) function
            result = expr
            if self.simplifier:
                result = self.simplifier.simplify(expr)
            syn = False
            if(expr==result):
                syn = True
            result = self.mySimplifier.simplify(result)
            # if self.simplifier.check_semantical_equivalence(result,result_mysimp):
            #     result = result_mysimp
            # if syn:
            #     result = self.synthesizer.simplify(result)
            # result = self.synthesizer.simplify(result)
             # simplifier should have simplify(expr) function
            # if result == expr:
            #     return self.visit(expr)
            return result
        result_mysimp = self.mySimplifier.simplify(expr)
        # if self.simplifier.check_semantical_equivalence(expr,result_mysimp):
        #     result = result_mysimp
        #     return result
        return self.visit(expr)

    def simplify_with_mySimpler(self, expr):
        result = expr
        result_mysimp = self.mySimplifier.simplify(expr)
        # if expr != result_mysimp:
        if expr != result_mysimp and self.simplifier.check_semantical_equivalence(expr, result_mysimp):
            return result_mysimp
        return None
    ...
    class Simplify():
        ...
        def simplify(self, expr):
            self.size = expr.size
            ret_expr = expr
            ret_expr = self.simp_slice_1(ret_expr)
            # ret_expr = self.simplify_mbablast(ret_expr)
            # if self.is_minus_expr(ret_expr):        # a ^ 0xFFFF + 1 -> -a
            #     if self.main_var is not None:
            #         ret_expr = ExprOp('-',self.main_var)
                    # ret_expr = ExprOp('!',self.main_var)
            return ret_expr

        def simp_slice_1(self,expr):
            ...
            if start == 31 and end == 32:
                ...
                return expr_simp(ExprOp("&",ExprInt(1,expr.arg.size),expr.arg))
            return expr


        def is_minus_expr(self, expr):
            one = ExprInt(1, self.size)
            if not expr.is_op():
                return False
            if expr.op != '+':
                return False
            if len(expr.args) != 2:
                return False
            if expr.args[0] != one and expr.args[-1] != one:
                return False

            if expr.args[0] == one:
                return self.xor_ffff(expr.args[-1])
            elif expr.args[-1] == one:
                return self.xor_ffff(expr.args[0])

        def xor_ffff(self, expr)->bool:
            fff = ExprInt(-1, self.size)
            if not self.is_binary_op(expr, '^'):
                return False
            if expr.args[0] != fff and expr.args[-1] != fff:
                return False
            if expr.args[0].is_int():
                self.main_var = expr.args[-1]
            if expr.args[-1].is_int():
                self.main_var = expr.args[0]
            return True

        def is_binary_op(self,expr, op)->bool:
            if not expr.is_op():
                return False
            if expr.op != op:
                return False
        ...
        def get_binary_op(self,x,op,y):
            return ExprOp(op,x,y)

        def rule_2_4(self,expr):
            if not self.is_binary_op(expr, '&'):
                return expr
            x = expr.args[0]
            y = expr.args[1]
            ret_expr = expr
            if self.xor_ffff(x) and not self.xor_ffff(y):
                ret_expr = ExprOp('+', y, -ExprOp('&',self.main_var,y))
            elif not self.xor_ffff(x) and self.xor_ffff(y):
                ret_expr = ExprOp('+', x, -ExprOp('&',x,self.main_var))
            return ret_expr

        def rule_6(self, expr):
            ret_expr = expr
            if not self.is_binary_op(expr, '^'):
                return expr
            x = expr.args[0]
            y = expr.args[1]
            x_plus_y = self.get_binary_op(x,'+',y)
            x_and_y = self.get_binary_op(x,'&',y)
            ret_expr = self.get_binary_op(x_plus_y, '+', -(x_and_y * ExprInt(2,expr.size)))
            return ret_expr

        def rule_7(self, expr):
            ret_expr = expr
            if not self.is_binary_op(expr, '|'):
                return expr
            x = expr.args[0]
            ...
            x_and_y = self.get_binary_op(x,'&',y)
            x_plus_y = self.get_binary_op(x, '+', y)
            ret_expr = self.get_binary_op(x_plus_y, '+', -x_and_y) # return x+y−(x∧y)
            return ret_expr

        def rule_8_9_14(self, expr):
            ret_expr = expr
            if not self.xor_ffff(expr):
                return expr
            if not expr.args[0].is_op():
                return expr
            sube = expr.args[0]
            x = sube.args[0]
            y = sube.args[1]
            x_and_y = self.get_binary_op(x, '&', y)
            mx_plus_my = self.get_binary_op(ExprOp('-',x), '+', ExprOp('-',y))
            if self.is_binary_op(expr.args[0],'|'):
                ret_expr = ExprOp('+', mx_plus_my, x_and_y, ExprOp('-',ExprInt(1, expr.size)))
            elif self.is_binary_op(expr.args[0], '^'):
                ret_expr = ExprOp('+', mx_plus_my, ExprOp('*', ExprInt(2,expr.size),x_and_y), ExprOp('-',ExprInt(1, expr.size)))
            elif self.is_binary_op(expr.args[0], '|'):
                ret_expr = ExprOp('+', ExprOp('-',x_and_y), ExprOp('-',ExprInt(1, expr.size)))

            return ret_expr

        def rule_11_13(self, expr):
            if not self.is_binary_op(expr, '|'):
                return expr
            x = expr.args[0]
            y = expr.args[1]
            ret_expr = expr
            if self.xor_ffff(x) and not self.xor_ffff(y):
                ret_expr = ExprOp('+', ExprOp('-', x), ExprOp('&',x,y), ExprOp('-', ExprInt(1,expr.size)))
            elif not self.xor_ffff(x) and self.xor_ffff(y):
                ret_expr = ExprOp('+', ExprOp('-', y), ExprOp('&',x,y), ExprOp('-', ExprInt(1,expr.size)))
            return ret_expr

        def rule_12(self,expr):
            ret_expr = expr
            if self.xor_ffff(ret_expr):             # a ^ 0xFFFF -> -a-1 (~a)
                if self.main_var is not None:
                    ret_expr = ExprOp('+',ExprOp('-',self.main_var), ExprOp('-', ExprInt(1, self.size)))
            return ret_expr


        def simplify_mbablast(self, expr):
            ret_expr = expr
            ret_expr = self.rule_2_4(ret_expr) # x∧ ¬y , ¬x∧y
            ret_expr = self.rule_12(ret_expr)
            ret_expr = self.rule_6(ret_expr) #  x⊕y
            ret_expr = self.rule_7(ret_expr) # x∨y
            ret_expr = self.rule_8_9_14(ret_expr) # ¬(x∨y), ¬(x⊕y), ¬(x∧y)
            ret_expr = self.rule_11_13(ret_expr) # x∨ ¬y, ¬x∨y

            return ret_expr


    def simpler_enum(self, expr, max = 50):
        """
        Main logic: currently it does nothing
        """
        # print ("# ESC: run your synthize based simplification result with expr, here--TODO!")
        ...
        if (expr.length() <  max):
            # result =  self.simplifier.simplify(expr) # simplifier should have simplify(expr) function
            result =  self.simplifier.simplify_enum(expr) # simplifier should have simplify(expr) function
            if result == expr:
                return self.visit(expr)
            return result
        return self.visit(expr)

    def simpler_enum_DB(self, expr, max = 50):
        """
        Main logic: currently it does nothing
        """
        # print ("# ESC: run your synthize based simplification result with expr, here--TODO!")
        ...
        if (expr.length() <  max):
            # result =  self.simplifier.simplify(expr) # simplifier should have simplify(expr) function
            result =  self.simplifier.simplify_enum_DB(expr) # simplifier should have simplify(expr) function
            if result == expr:
                return self.visit(expr)
            return result
        return self.visit(expr)
    ...
    def visit(self, expr, *args, **kwargs):
# ESC20210709 : base case
        if expr.is_int() or expr.is_id() or expr.is_loc():
            ret = expr
# ESC20210709 : modified
        elif expr.is_assign():
            ret = self.simpler(expr.src)
        # ESC20210709: dummy
        elif expr.is_cond():
            cond = self.visit(expr.cond, *args, **kwargs)
            src1 = self.visit(expr.src1, *args, **kwargs)
        ...
        # ESC20210709: dummy
            ...
            ptr = self.visit(expr.ptr, *args, **kwargs)
            ret = ExprMem(ptr, expr.size)
            self.representation[ret] = ExprId(f"mem_{self.rep_count}", expr.size)
            self.rep_count += self.rep_count
        # ESC20210709: dummy
        elif expr.is_slice():
            arg = self.visit(expr.arg, *args, **kwargs)
            ret = ExprSlice(arg, expr.start, expr.stop)
            ret = self.simpler(ret)
# ESC20210709: modified
        elif expr.is_op():
            args = [self.simpler(arg) for arg in expr.args]
            ret = ExprOp(expr.op, *args)
            # self.simplifier.updateTable(ret)
            # ret = self.simpler(ExprOp(expr.op, *args))
        # ESC20210709: dummy
        elif expr.is_compose():
            args = [self.visit(arg, *args, **kwargs) for arg in expr.args]
            ret = ExprCompose(*args)
    ...
    def get_unification_candidates(self, expr):
        """
...
class ExprMem(Expr):
    ...
    def __init__(self, ptr, size=None):
        ...
        # size = 32 #edited for BitVec compatible Error
...
class ExprOp(Expr):
    ...
    def length(self):
        length = [arg.length() for arg in self._args]
        if self._op=='-':
            return sum(length)+1
        return sum(length) + len(self._args) - 1
...
class ExprCompose(Expr):
    ...
    def length(self):
        depth = [arg.length() for arg in self._args]
        return sum(depth) + 1
