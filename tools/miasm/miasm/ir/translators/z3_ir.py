...
class TranslatorZ3(Translator):
    ...
    def from_ExprInt(self, expr):
        return z3.BitVecVal(int(expr), 32)

    def from_ExprId(self, expr):
        return z3.BitVec(str(expr), 32)
    ...
    def from_ExprLoc(self, expr):
        ...
        if offset is not None:
            return z3.BitVecVal(offset, 32)
        # fallback to default name
        return z3.BitVec(str(loc_key), 32)

    def from_ExprMem(self, expr):
        addr = self.from_expr(expr.ptr)
        return self._mem.get(addr, 32)

    def from_ExprSlice(self, expr):
        res = self.from_expr(expr.arg)
        res = z3.Extract(32-1, 0, res)
        return res
